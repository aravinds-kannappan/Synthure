...
├── eval/                              # Benchmarks of the previous pipeline (see note in the dir)
...
The scripts in `eval/` measured the **previous** pipeline (Claude tool_use NER versus a regex baseline, and a lexical retriever versus BM25). The shipped pipeline now extracts with OpenMed models on device and links codes through the official ICD 10 CM index with constrained choice, so those numbers no longer describe what runs. Re benchmarking the current pipeline (entity span F1 against a de identified clinical corpus, and code linking accuracy against the index) is the first open item below. The eval scripts are kept as the record of the prior measurement.
