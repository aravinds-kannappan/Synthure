const API_BASE = ''; // change to 'http://localhost:5050' for local dev
let token = null;
...
document.getElementById('login-form').addEventListener('submit', async (e) => {
  ...
  try {
    ...
    token = data.token;
    enterApp(data.name || 'Dr. Sarah Chen');
  } catch (ex) {
...
document.getElementById('signup-form').addEventListener('submit', async (e) => {
  ...
  try {
    // Use demo credentials to get a real JWT; display name comes from signup form
    const res  = await fetch(`${API_BASE}/api/auth/login`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ email: 'demo@synthure.ai', password: 'demo1234' }),
    });
    const data = await res.json();
    if (!res.ok) throw new Error('Unable to create account right now. Please try the demo credentials.');
    token = data.token;
    enterApp(name);
  } catch (ex) {
    err.textContent = ex.message;
...
document.getElementById('logout-btn').addEventListener('click', backToLanding);
...
function scoreRingSVG(score, cls) {
  ...
  const colors = { 'score-high': '#34d399', 'score-med': '#fbbf24', 'score-low': '#4a6080' };
  return `<svg width="46" height="46" viewBox="0 0 46 46">
    ...
    <circle cx="${c}" cy="${c}" r="${r}" fill="none" stroke="${colors[cls]||'#4a6080'}" stroke-width="3"
    ...
    return `<div class="trace-step">
      ...
      <div class="trace-step-body">
        <div class="trace-step-name">
          ...
          <span style="color:${hasIssues?'var(--yellow)':'var(--green)'};font-size:9px;font-weight:700">${hasIssues?'⚠':'✓'}</span>
        </div>
...
async function explainJargon() {
  ...
  const btn = document.getElementById('jargon-btn'); const result = document.getElementById('jargon-result');
  ...
  el.innerHTML = `<div class="result-body">
    ...
    ${d.medications?.length ? `<div><div class="r-section-label">Medications</div>
      ${d.medications.map(m => `<div class="med-item">
        <div class="med-name">${m.name}</div><div class="med-purpose">${m.purpose}</div><div class="med-instructions">${m.instructions}</div>
      </div>`).join('')}
    </div>` : ''}
...
async function matchInsurance() {
  ...
  const btn = document.getElementById('insurance-btn'); const result = document.getElementById('insurance-result');
  btn.disabled = true; setLoading(result);
  try {
    ...
    <div><div class="r-section-label">Best Matches</div>
      ${recommendations.map((r, i) => {
        ...
        return `<div class="match-card ${cls}">
          ...
          <div class="match-info"><div class="match-plan">${i===0?'★ ':''}${r.plan}</div><div class="match-reason">${r.reason}</div></div>
        </div>`;
      }).join('')}
    </div>
...
async function submitClaim() {
  ...
  const btn = document.getElementById('claim-btn'); const result = document.getElementById('claim-result');
  btn.disabled = true; setLoading(result);
  try {
...
function renderClaim(el, data) {
  ...
  el.innerHTML = `<div class="result-body">
    <div class="decision-banner decision-${r.decision}">
      ...
      <div><div class="decision-label">${labels[r.decision]}</div><div class="decision-claim-id">${claim_id}</div></div>
